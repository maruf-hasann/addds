...
const EditTutoringPlaceModal = ({
  setEditTutoringPlaceData,
  editTutoringPlaceData,
  setOpenEditTutoringPlaceModal,
  openEditTutoringPlaceModal,
}) => {

  const editTutoringPlace = null
  const isLoading = false


  // handle close modal
  const handleClose = () => {
    setEditTutoringPlaceData(null);
    setOpenEditTutoringPlaceModal(!openEditTutoringPlaceModal);
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    const placeName = e.target.name.value;
    const result = await editTutoringPlace({ placeName });
  ...
  return (
    <div
      className={`fixed top-0 left-0 z-50 p-4 overflow-x-hidden overflow-y-auto inset-0 h-[calc(100%-1rem)] max-h-full backdrop-blur-sm bg-white/5 ${
        openEditTutoringPlaceModal ? "block" : "hidden"
      }`}
    >
...
export default EditTutoringPlaceModal;
