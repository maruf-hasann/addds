...
const EditMockPricingModal = ({
  ...
  const mainSubjects = mainSubjectsData?.data?.filter(
    (mainSubject) =>
      mainSubject?.subjectVariant?.toLowerCase() === "Academic".toLowerCase()
  );
  ...
  const gradeData = allGradeData?.data?.filter(
    (grade) =>
      grade?.educationVariant?.toLowerCase() === educationVariant?.toLowerCase()
  );
  /* react hook form */
  ...
  // sub subject
  const [getSubSubjectByMainSubject] = useLazyGetSubSubjectByMainSubjectQuery();
            ...
            <div className="py-10">
              ...
              >
                <div>
                  ...
                  >
                    <option value={editData?.educationVariant}>
                      {editData?.educationVariant}
                    </option>
                    {educationVariants
                      ?.filter(
                        (variant) =>
                          variant?.variantName !== editData?.educationVariant
                      )
                      ?.map((variant, idx) => (
                ...
                {gradeData?.length ? (
                  <div>
                    <label
                      ...
                      className="block mb-2 font-semibold text-sm text-gray-500"
                    >
                      {educationVariant === "English Medium"
                        ? "Grade"
                        : "Class"}
                    </label>
                    <select
                      ...
                      className="w-full p-2 mb-4 border rounded-md outline-none focus:outline-primaryAlfa-50"
                    >
                      <option value={editData?.grade}>{editData?.grade}</option>
                      {gradeData
                        ?.filter(
                          (grade) => grade?.className !== editData?.grade
                        )
                        ?.map((grade, idx) => (
                          <option key={idx} value={grade?.className}>
                            {grade?.className}
                          </option>
                        ))}
                    </select>
                ...
                <div>
                  ...
                  >
                    ...
                    {mainSubjects
                      ?.filter(
                        (mainSubject) =>
                          mainSubject?.name !== editData?.mainSubject
                      )
                      ?.map((mainSubject, idx) => (
                        <option key={idx} value={mainSubject?.name}>
                          {mainSubject?.name}
                        </option>
                      ))}
                  </select>
                ...
                {selectedSubSubjects?.length ? (
                  <div>
                    ...
                    >
                      {editData?.subSubject ? (
                        <option value={editData?.subSubject}>
                          {editData?.subSubject}
                        </option>
                      ) : (
                        <option value={""}>Please select sub subject</option>
                      )}

                      {selectedSubSubjects
                        ?.filter(
                          (mainSubject) =>
                            mainSubject?.subSubject !== editData?.subSubject
                        )
                        ?.map((subSubject, idx) => (
                          <option key={idx} value={subSubject?.subSubject}>
