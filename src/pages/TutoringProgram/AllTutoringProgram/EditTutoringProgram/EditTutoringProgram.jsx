...
const EditTutoringProgramModal = ({
  setEditTutoringProgramData,
  editTutoringProgramData,
  setOpenEditTutoringProgramModal,
  openEditTutoringProgramModal,
}) => {

  const editTutoringProgram = null
  const isLoading = false


  // handle close modal
  const handleClose = () => {
    setEditTutoringProgramData(null);
    setOpenEditTutoringProgramModal(!openEditTutoringProgramModal);
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    const programName = e.target.name.value;
    const result = await editTutoringProgram({ programName });
  ...
  return (
    <div
      className={`fixed top-0 left-0 z-50 p-4 overflow-x-hidden overflow-y-auto inset-0 h-[calc(100%-1rem)] max-h-full backdrop-blur-sm bg-white/5 ${
        openEditTutoringProgramModal ? "block" : "hidden"
      }`}
    >
        ...
        >
          ...
          >
            ...
            <input
              ...
              name="name"
              required
              defaultValue={editTutoringProgramData?.programName}
              placeholder="Trainer"
...
export default EditTutoringProgramModal;
