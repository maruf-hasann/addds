...
import { useAddCurriculumBoardMutation } from "../../../../store/service/curriculumBoard/curriculumBoardApiService";

const AddCurriculumBoardModal = ({
  openAddCurriculumBoardModal,
  setOpenAddCurriculumBoardModal,
}) => {
  const [curriculumBoardName, setCurriculumBoardName] = useState("");
  const [educationVariant, setEducationVariant] = useState("");

  const [addCurriculumBoard, { isLoading }] = useAddCurriculumBoardMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!curriculumBoardName)
      return toast.error("Please add a curriculum Board name");
    if (!educationVariant) return toast.error("Please Add a education variant");

    const result = await addCurriculumBoard({
      boardName: curriculumBoardName,
      educationVariant,
    });
    console.log(result);
    if (result?.data?.success) {
      toast.success(result?.data?.message);
      setCurriculumBoardName("");
      setEducationVariant("");
      setOpenAddCurriculumBoardModal(!openAddCurriculumBoardModal);
    } else {
  ...
  return (
    <div
      className={`fixed top-0 left-0 z-50 p-4 overflow-x-hidden overflow-y-auto inset-0 h-[calc(100%)] max-h-full backdrop-blur-sm ${
        openAddCurriculumBoardModal ? "block" : "hidden"
      }`}
    >
      <div
            ...
            <div className="py-10">
              ...
              <form className="max-w-md mx-auto p-4 border rounded-md mt-5 bg-white">
                <div>
                  ...
                  <input
                    type="text"
                    id="curriculumBoardName"
                    name="curriculumBoardName"
                    value={curriculumBoardName}
                    onChange={(e) => setCurriculumBoardName(e.target.value)}
                    required
...
export default AddCurriculumBoardModal;
