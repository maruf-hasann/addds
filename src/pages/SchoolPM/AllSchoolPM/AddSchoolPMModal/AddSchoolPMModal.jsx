...
import { useAddSchoolPMMutation } from "../../../../store/service/schoolPM/schoolPMApiService";
import { useGetEducationVariantsQuery } from "../../../../store/service/educationVariant/educationVariantApiService";

const AddSchoolPMModal = ({
  openAddSchoolPMModal,
  setOpenAddSchoolPMModal,
}) => {
  const [schoolName, setSchoolName] = useState("");
  const [educationVariant, setEducationVariant] = useState("");

  const [addSchoolPM, { isLoading }] = useAddSchoolPMMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!schoolName) return toast.error("Please add a school name");
    if (!educationVariant) return toast.error("Please Add a education variant");

    const result = await addSchoolPM({ schoolName, educationVariant });
    console.log(result);
    if (result?.data?.success) {
      toast.success(result?.data?.message);
      setSchoolName("");
      setEducationVariant("");
      setOpenAddSchoolPMModal(!openAddSchoolPMModal);
    } else {
  ...
  return (
    <div
      className={`fixed top-0 left-0 z-50 p-4 overflow-x-hidden overflow-y-auto inset-0 h-[calc(100%)] max-h-full backdrop-blur-sm ${
        openAddSchoolPMModal ? "block" : "hidden"
      }`}
    >
      <div
          ...
          <>
            <div className="py-10">
              <div className="flex justify-between items-center pb-3">
                <h1 className="font-bold text-blue-gray-800">Add School PM</h1>
              </div>
...
export default AddSchoolPMModal;
